    ...
    def load_state(self):
        if self.state_file.exists():
            try:
                ...
                d.setdefault("history",[]); d.setdefault("completed_actions",[]); d.setdefault("action_attempts",{}); d.setdefault("mission_complete",False); return d
            except Exception as e:logger.error("[-] state read: %s",e)
        return {"target":self.target_ip,"phase":"recon","step_count":0,"mission_complete":False,"last_output":"","history":[],"discovered_services":[],"vulnerabilities":[],"credentials":[],"completed_actions":[],"action_attempts":{}}
